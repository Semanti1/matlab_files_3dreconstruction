...
cx = mean(xpin);
cy = mean(ypin);
a = atan2(ypin - cy, xpin - cx);
[~, order] = sort(a);
xpin = xpin(order);
ypin = ypin(order);
arr=round(linspace(1,size(pin,1),100));
...
bound_orig2=unique(bound_orig2,'rows');
xbound_orig2=bound_orig2(:,1);
ybound_orig2=bound_orig2(:,2);
cx = mean(xbound_orig2);
cy = mean(ybound_orig2);
a = atan2(ybound_orig2 - cy, xbound_orig2 - cx);
[~, order] = sort(a);
xbound_orig2 = xbound_orig2(order);
ybound_orig2 = ybound_orig2(order);
arr=round(linspace(1,size(bound_orig2,1),100));

bound_orig2=[xbound_orig2,ybound_orig2];
bound_orig2=bound_orig2(arr,:);
% aff_tr = [(y(2,:))',(y(1,:))'];
aff_tr = y'
c1 = [mean(y2),mean(x2)];
...
hold on;
plot(aff_tr_trans(:,1),aff_tr_trans(:,2),'gx')
%%%%%%Start after here with procrustes
%%
% hold on;
% plot(y2,x2,'rx');
...
% dist = pdist2(boundpts_aff,boundpts_orig);
[v,ind]=min(dist,[],2);
all_img1 = x';
